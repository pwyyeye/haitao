...
-(void)reloadCollapseClick {
    ...
    // Add cells
    for (int i = 0; i < [CollapseClickDelegate numberOfCellsForCollapseClick]; i++) {
        // Create Cell
        CollapseClickCell *cell = [CollapseClickCell newCollapseClickCellWithTitle:[CollapseClickDelegate titleForCollapseClickAtIndex:i] index:i content:[CollapseClickDelegate viewForCollapseClickContentViewAtIndex:i] withHeight:_cellHeight];
        ...
        if ([(id)CollapseClickDelegate respondsToSelector:@selector(colorForCollapseClickTitleViewAtIndex:)]) {
            cell.TitleView.backgroundColor = [CollapseClickDelegate colorForCollapseClickTitleViewAtIndex:i];
        }
        else {
        ...
        if ([(id)CollapseClickDelegate respondsToSelector:@selector(colorForTitleLabelAtIndex:)]) {
            cell.TitleLabel.textColor = [CollapseClickDelegate colorForTitleLabelAtIndex:i];
        }
        else {
...
//            [cell.TitleArrow drawWithColor:[CollapseClickDelegate colorForTitleArrowAtIndex:i]];
//        }
//        else {
...
//        }
        ...
        cell.ContentView.frame = CGRectMake(0, _cellHeight + _cellSpace, self.frame.size.width, cell.ContentView.frame.size.height);
        
        // Set cell's size
        cell.frame = CGRectMake(0, totalHeight, self.frame.size.width, _cellHeight);
        ...
        totalHeight += _cellHeight + _cellSpace;
    }
...
-(void)repositionCollapseClickCellsBelowIndex:(int)index withOffset:(float)offset {
    ...
    [self setContentSize:CGSizeMake(self.frame.size.width, lastCell.frame.origin.y + lastCell.frame.size.height + _cellSpace)];
}
...
-(void)openCollapseClickCellAtIndex:(int)index animated:(BOOL)animated {
    ...
    if ([[self.isClickedArray objectAtIndex:index] boolValue] != YES) {
        ...
        [UIView animateWithDuration:duration animations:^{
            ...
            cell.frame = CGRectMake(cell.frame.origin.x, cell.frame.origin.y, cell.frame.size.width, cell.ContentView.frame.origin.y + cell.ContentView.frame.size.height + _cellSpace);
            
            // Change Arrow orientation
//            CGAffineTransform transform = CGAffineTransformMakeRotation(M_PI/2);
//            cell.TitleArrow.transform = transform;
            UIImageView *imageView= cell.TitleArrow.subviews[0];
        
            imageView.image=[UIImage imageNamed:@"icon_Drop-downList"];
            ...
            [self repositionCollapseClickCellsBelowIndex:index withOffset:cell.ContentView.frame.size.height + _cellSpace];
        }];
    }
}
...
-(void)closeCollapseClickCellAtIndex:(int)index animated:(BOOL)animated {
    ...
    if ([[self.isClickedArray objectAtIndex:index] boolValue] == YES) {
        ...
        [UIView animateWithDuration:duration animations:^{
            // Resize Cell
            CollapseClickCell *cell = [self.dataArray objectAtIndex:index];
            cell.frame = CGRectMake(cell.frame.origin.x, cell.frame.origin.y, cell.frame.size.width, _cellHeight);
...
//            cell.TitleArrow.transform = transform;
            ...
            // Reposition all CollapseClickCells below Cell
            [self repositionCollapseClickCellsBelowIndex:index withOffset:-1*(cell.ContentView.frame.size.height + _cellSpace)];
        }];
    }
}
