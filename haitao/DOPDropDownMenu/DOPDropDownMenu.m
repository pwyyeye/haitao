...
//        CALayer *bgLayer = [self createBgLayerWithColor:[UIColor whiteColor] andPosition:bgLayerPosition];
        ...
        CALayer *bgLayer;
        if(i==0){
            bgLayer = [self createBgLayerWithColor:[UIColor whiteColor] andPosition:bgLayerPosition];
        }else{
            bgLayer = [self createBgLayerWithColor:_backgroundViewColor andPosition:bgLayerPosition];
        }
        ...
        //modify by pwy --判断纪录是否为一笔 如果为一笔 则不显示下拉箭头  改变颜色 与背景色相同--begin
        if ([self.dataSource respondsToSelector:@selector(menu:numberOfRowsInColumn:)]) {
            if ([self.dataSource menu:self numberOfRowsInColumn:i]!=1) {//不为一笔才有下下拉箭头
                [self.layer addSublayer:indicator];
            }
        }
        //modify by pwy --判断纪录是否为一笔 如果为一笔 则不显示下拉箭头  改变颜色 与背景色相同--end
...
//        _currentSelectedMenudIndex = -1;
        _currentSelectedMenudIndex = 0;// modify by pwy
...
//            [(CALayer *)self.bgLayers[i] setBackgroundColor:[UIColor whiteColor].CGColor];
            [(CALayer *)self.bgLayers[i] setBackgroundColor:_backgroundViewColor.CGColor];
        }
    }
    
    //modify by pwy --判断纪录是否为一笔 如果为一笔 则不能下拉箭头  改变颜色 与背景色相同--begin
    BOOL onlyOneCount=NO;
    if ([self.dataSource respondsToSelector:@selector(menu:numberOfRowsInColumn:)]) {
    ...
    if (tapIndex == _currentSelectedMenudIndex && _show && onlyOneCount==NO) {
...
//        [(CALayer *)self.bgLayers[tapIndex] setBackgroundColor:[UIColor whiteColor].CGColor];
        [(CALayer *)self.bgLayers[tapIndex] setBackgroundColor:_backgroundViewColor.CGColor];
    } else if(onlyOneCount==NO){
        _currentSelectedMenudIndex = tapIndex;
...
- (void)backgroundTapped:(UITapGestureRecognizer *)paramSender
{
    ...
    //pwy modify
//    [(CALayer *)self.bgLayers[_currentSelectedMenudIndex] setBackgroundColor:_backgroundViewColor.CGColor];
